...
import { isTruthyLike, normalizePageId, nowIso, sleep } from "./utils.js";
...
function collectPageIds(fields, fieldNames) {
  const output = [];
  const seen = new Set();
  for (const fieldName of fieldNames) {
    ...
    for (const piece of pieces) {
      const pageId = normalizePageId(piece);
...
function normalizeCheckpoint(record) {
  ...
  return {
    ...
    pageId: firstText(fields[CHECKPOINT_FIELDS.pageId]),
    pageName: firstText(fields[CHECKPOINT_FIELDS.pageName]),
    searchQuery: firstText(fields[CHECKPOINT_FIELDS.searchQuery]),
    cursor: firstText(fields[CHECKPOINT_FIELDS.cursor]),
    position: toNumber(fields[CHECKPOINT_FIELDS.position], 0),
    lastLibraryId: firstText(fields[CHECKPOINT_FIELDS.lastLibraryId]),
...
export class AirtableClient {
  ...
  async getCompetitorPageSeeds() {
    ...
    for (const record of records) {
      ...
      const pageName = firstText(
        fields[this.config.airtable.competitorsNameField],
        fields.Name,
        fields.name
      );

      for (const pageId of collectPageIds(fields, pageIdFields)) {
        if (seen.has(pageId)) {
          continue;
        }
        seen.add(pageId);
  ...
  async getCheckpointState({ retryLimit, staleMinutes }) {
    ...
    return {
      ...
      staleRunning,
      retryableFailed,
      checkpoints: [...checkpoints].sort(checkpointSort).slice(0, 200)
    };
  }
    ...
    for (const row of rows) {
      ...
      creates.push({
        fields: compactFields({
          ...
          [CHECKPOINT_FIELDS.pageId]: row.pageId,
          [CHECKPOINT_FIELDS.pageName]: row.pageName,
          [CHECKPOINT_FIELDS.searchQuery]: row.searchQuery,
          [CHECKPOINT_FIELDS.cursor]: row.cursor,
          [CHECKPOINT_FIELDS.position]: row.position,
          [CHECKPOINT_FIELDS.lastLibraryId]: row.lastLibraryId,
